...
import { Quiz, RoomState, SyncMessage } from './types.js';
...
const socketRoomMap = new Map<string, Set<string>>();

// Host tracking per room for race condition protection: roomCode -> { socketId, hostTabId }
const roomHostMap = new Map<string, { socketId: string; hostTabId: string }>();
...
app.post('/api/quizzes', (req: Request, res: Response): void => {
  try {
...
app.delete('/api/quizzes/:id', (req: Request, res: Response): void => {
...
app.post('/api/generate-quiz', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const safeConfigLog = {
      baseUrl: aiConfig?.baseUrl || '(default / server env)',
      model: aiConfig?.model || '(default / server env)',
      hasClientApiKey: Boolean(aiConfig?.apiKey?.trim()),
    };
    console.log(
      `[AI Generator] Request received for topic: "${topic}" (${questionCount || 5} questions, lang: ${language || 'ru'}), aiConfig:`,
      safeConfigLog
    );
...
app.post('/api/test-ai-connection', async (req: Request, res: Response): Promise<void> => {
  try {
...
io.on('connection', (socket: Socket) => {
  ...
  // Handle room joining
  socket.on('join_room', (roomCode: string, playerId?: string) => {
    if (!roomCode) return;
  ...
  socket.on('sync_message', (msg: SyncMessage) => {
