...
- **Многопользовательский режим в реальном времени** через WebSocket (Socket.IO) между любыми устройствами (телефоны, ноутбуки)
- **ИИ-генератор вопросов (BYOK)**: генерация викторин через Google Gemini REST API или любой OpenAI-совместимый эндпоинт (Groq, DeepSeek, Ollama)
- **Бесшовный офлайн-фолбэк** на шаблоны при отсутствии сети или ключей
- **Локальное хранилище** — все созданные квизы и история сохраняются в браузере (LocalStorage)
- **Звуковые эффекты** для событий игры (клики, правильные/неправильные ответы, фанфары)
- **Адаптивный дизайн** на TailwindCSS и анимации на Motion
...
Подробная инструкция по запуску сервера и подключению с телефонов: [SERVER_SETUP.md](./SERVER_SETUP.md).

### 1. Запуск Backend-сервера (WebSocket + AI)
```bash
cd server
npm install
npm run dev
